...
exports.postSignUp = [
  ...
  body("password", "Password cannot be empty.")
    ...
    .isLength({ min: 1 })
    .escape(),
  body("adminkey", "Admin key must be 'adminadmin'.").custom((value) => {
    if (value !== "adminadmin") {
  ...
  (req, res, next) => {
    ...
    // Check if the provided adminkey is "adminadmin"
    if (adminkey !== "adminadmin") {
    ...
    User.findOne({ username }, (err, user) => {
      ...
      try {
        bcrypt.hash(password, 10, (err, hashedPassword) => {
          ...
          User.create(
            { username, password: hashedPassword, adminkey },
            (err, user) => {
